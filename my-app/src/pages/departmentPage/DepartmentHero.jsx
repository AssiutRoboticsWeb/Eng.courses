...
const DepartmentHero = ({ departmentDetails, setYear, setYearId, setSemester }) => {
  // console.log("depart hero ");
      ...
      bg-cover bg-center">
          ...
          <div className="content absolute top-0 left-0 w-full h-full flex  flex-col gap-10 items-center justify-center">
            <h2 className="text-3xl md:text-5xl font-bold text-white">
              {departmentDetails ? departmentDetails.data.name : "Loading..."}
            </h2>
            <aside className="flex gap-5">
              <aside className="flex flex-col items-center gap-1">
                ...
                <select
                  ...
                  {departmentDetails &&
                    departmentDetails.data.years.map((year, index) => (
                      setYear(year.name),
                      <option key={index} value={year._id}>
                        {year.name ? (year.name).split(" ")[1] : "Loading..."}
                      </option>
                    ))}
                </select>
              </aside>
